...
class DeepGCN(torch.nn.Module):
    def __init__(self, opt):
        ...
        for i in range(n_b):
            if i > 0:
                ...
                self.reverse_backbone.append(Upsample(channels[n_b-i-1],channels[n_b-i]))
                HW = HW // 4
                # print(HW_up)
            for j in range(blocks[i]):
                ...
                self.reverse_backbone += [
                    Seq(
                        FFN(channels[i], channels[i], act=act, drop_path=dpr[idx]),
            ...
            if i>0:
                HW_up = HW_up//4
        print(idx,idx2)
        # self.backbone.append(nn.Conv2d(384,240,1,bias=True))
    ...
    def forward(self, inputs):
        ...
        for i in range(len(self.reverse_backbone)-1,-1,-1):
            # print(i)
